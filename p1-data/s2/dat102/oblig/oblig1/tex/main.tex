...
Vi teller antall tilordninger ($=$), antall ganger ($=$) brukes.

1) Utenfor løkken: 1 tilordning \lstinline{sum = 0;}

2) I "for"-løkken: 1 tilordning \lstinline{int i = n;}

3) I "for"-løkken: 1 tilordning \lstinline{i = i/2;}

4) I "for"-løkken: 1 tilordning \lstinline{sum = sum + i;}

$\Rightarrow$ Total antall iterasjoner kan bli beskrivet som "$\log_2 (n)$" ganger.

Inn i løkken "\lstinline{sum = 0}" = 1, "\lstinline{i = n}" = 1 $\rightarrow 2 \cdot log_2 (n)$
...
Teller hver gang ($=$) brukes.

1) "\lstinline{sum = 0;}" $\rightarrow$ 1. Tilordning.

2) "\lstinline{int i = 1;}" $\rightarrow$ 1. Tilordning.

3) "\lstinline{i++}" $\rightarrow$ Skjer $n$ ganger, pga skjer hele tiden.

4) "\lstinline{int j = 1}" $\rightarrow$ Avhengig av ytre-løkke, så den skjer en gang per ytre iterasjon. $n$ ganger.

5) "\lstinline{j = j * 2}" $\rightarrow$ Avhengig av ytre-løkke, skjer $\log_2 (n)$ ganger per ytre iterasjon pga. multiplikasjon. $n \cdot \log_2 (n)$.

6) "\lstinline{sum = sum + i \cdot j}" $\rightarrow$ $n \cdot \log_2 (n)$ ganger.
