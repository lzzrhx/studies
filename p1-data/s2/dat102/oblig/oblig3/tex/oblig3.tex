...
\newcommand{\oppgaveDelStart}{\begin{enumerate}[leftmargin=*,itemsep=0.7cm,labelsep=1.5em,label=\alph*)]}
...
\titlespacing*{\subsubsection}{0pt}{3.25ex plus 1ex minus .2ex}{0.5ex}
...
\begin{enumerate}[leftmargin=*,itemsep=1cm,labelsep=1.5em,label=\roman*.]
    \item
        ...
        \par $x \in A$
        \par For både \code{TabellMengde} og \code{LenketMengde} vil metoden gå gjennom alle elementer frem til det gitte elementet er funnet, eller konkludere med at det ikke finnes i mengden hvis det ikke ble funnet etter gjennomgang av alle elementer. I beste tilfelle er mengden tom og løkken der søket gjennomføres hoppes over, dette gir $O(1)$ i O-notasjon. I verste tilfelle har vi en mengde som inneholder elementer og det letes etter et element som ikke ekisterer i mengden. Da må alle elementer sjekkes og dette gir $O(n)$ i O-notasjon, der $n$ er antall elementer i mengden.
    \item
    ...
    \item
        ...
        \par $A - \{x\}$
        \par Denne metoden fungerer også svært likt for begge klassene. Gitt en mengde $A$ og et element $x$ som skal fjernes fra mengden, gjennomføres det en iterasjon gjennom mengden for å finne $x$. Dette kan gi to ulike resultater. 1) Hvis $x$ blir funnet fjernes $x$ fra mengden og $x$ returneres. 2) Hvis alle elementer i $A$ blir sjekket uten at $x$ ble funnet returneres \code{null}. I worst case finnes ikke $x$ i $A$ og alle elementer må sjekkes. Dette gir $O(n)$ i O-notasjon. I best case er $A$ en tom mengde og \code{null} returneres umiddelbart, som gir $O(1)$ i O-notasjon.
\end{enumerate}

\oppgaveDel{e}
    \par HashSet bruker en HashMap (også kalt dictionary i noen språk) datastruktur for å lagre verdier i en tabell.
    Gitt en mengde med en underliggende tabell med kapasiteten $n$ og en verdi $x$ som vi ønsker å lagre, vil det gjennomføres en hash-metode på $x$ som gir heltallsverdien $x_h$.
    Indeksverdien $x_i = x_h \mod n$ brukes for å direkte referere til posisjonen i den underliggende tabellen hvor $x$ skal lagres.
    I noen tilfeller vil vi få flere verdier med samme indeksverdi, og dette kalles kollisjon.
    Kollisjoner må løses slik at verdiene får tildelt unike indeksverdier.
    Dette er en forenklet forklaring på hvordan en HashMap datastruktur fungerer (uten å beskrive hvordan kollisjoner løses).
    Likevel ser vi utifra dette at det blir mye raskere å sjekke om et element eksisterer eller ikke i en gitt mengde sammenlignet med vår naive \code{inneholder(...)} metode fra \code{TabellMengde} og \code{LenketMengde}.
    En \code{inneholder(T element)} metode på et sett som bruker en HashMap datastruktur vil ha en kjøretid på $O(1)$ uttrykt i O-notasjon.

\oppgaveDel{f} Se vedlagte filer \code{./src/Person.java} og \code{./src/Program.java} (match resultat går på skala fra 0.0 (dårlig match) til 1.0 (perfekt match)).

\oppgaveDelSlutt

\section*{Uke 11 \textendash \ Oppgave 4}
\par Vi opprettet en \code{HashSet<Integer>} mengde med 100.000 unike, tilfeldig valgte heltall fra domenet $\{0 \ .. \ 999999\}$, og en sortert \code{Integer[]} tabell med 100.000 tilfeldig valgte heltall fra samme domenet, der duplikate verdier i tabellen var tillatt.
Videre gjennomførte vi kjøretidsmåling for å finne antall forekomster av 10.000 tilfeldig valgte heltall i \code{HashSet} mengden og i \code{Integer} tabellen.
For generering av tilfeldige heltallsverdier implementerte vi en linear-feedback shift register (LFSR) klasse.

\subsubsection*{Fremgangsmåte i \code{HashSet<Integer>} mengden:}
\code{HashSet} implementerer \code{Set}-grensesnittet, som ikke tillater duplikate verdier og har en innebygd \code{boolean contains(T element)} metode. Så her holdt det å gjøre metodekall for alle de 10.000 verdiene på \code{mengde.contains(verdi)} der \code{true} telles som $1$ forekomst av den gitte verdien i mengden og \code{false} telles som $0$ forekomster.

\subsubsection*{Fremgangsmåte i \code{Integer[]} tabellen:}
For den sorterte \code{Integer} tabellen gjennomførte vi binærsøk for å finne ut om en gitt verdi eksisterte i tabellen. Hvis verdien ble funnet gjennomførte vi en telling av naboelementer med samme verdi for å finne totalt antall forekomster av den gitte verdien i tabellen.
Vi implementerte en rekursiv- og en iterativ binærsøkemetode og tok målinger fra begge. 

\subsubsection*{Resultat:}
Det ble målt kjøretid på $\approx 0,5 \ ms$ for telling av antall forekomster i \code{HashSet} mengden.
Rekursivt binærsøk i \code{Integer} tabellen ga kjøretid på $\approx 5,5 \ ms$ og iterativt binærsøk ga kjøretid på $\approx 4,5 \ ms$.
Den raske kjøretiden for \code{HashSet} stemmer overens med resultatet vi hadde forventet ettersom HashMap datastrukturen har en kjøretid på $O(1)$ for å finne ut om en gitt verdi eksisterer i den underliggende tabellen.

\subsubsection*{Kildekode:}
Se vedlagte filer \code{./src/BinSok.java}, \code{./src/Lfsr.java}, \code{./src/Program.java} og \code{./test/BinSokTest.java} for implementasjon brukt til målingene.
