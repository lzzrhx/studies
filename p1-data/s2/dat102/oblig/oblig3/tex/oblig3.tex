...
Videre gjennomførte vi kjøretidsmåling på å finne antall forekomster av 10.000 tilfeldig valgte heltall i \code{HashSet} mengden og i \code{Integer} tabellen.
...
For den sorterte \code{Integer} tabellen gjennomførte vi binærsøk for å finne ut om en gitt verdi eksisterer i tabellen. Hvis verdien ble funnet gjennomførte vi en telling av naboelementer med samme verdi for å finne totalt antall forekomster av den gitte verdien i tabellen.
...
Se vedlagte filer \code{./src/BinSok.java}, \code{./src/Lfsr.java}, \code{./src/Program.java} og \code{./test/BinSokTest.java} for implementasjon for gjennomføring av målingene.
