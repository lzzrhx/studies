...
\newcommand{\forfatter}{Casper Eide Özdemir-Børretzen}
...
\oppgaveDel{a} Se vedlagte filer src/TabellMengde.java og test/TabellMengdeTest.java
\oppgaveDel{b} Se vedlagte filer src/LenketMengde.java og test/LenketMengdeTest.java
\oppgaveDel{c} Se vedlagte filer src/JavaSetToMengde.java og test/JavaSetToMengdeTest.java
\oppgaveDel{d}
...
\begin{enumerate}[leftmargin=*,itemsep=1cm,labelsep=1.5em,label=\roman*.]
    ...
    \item
        ...
        \par Metoden fungerer veldig likt for begge klassene, og i begge tilfeler sammenlignes to mengder. La oss kalle de mengde $A$ og mengde $B$. I metoden undersøkes det om $A$ er delmengde av $B$. Først blir det utført en sjekk for å forsikre at antall elementer i $A$ er like eller mindre enn antall elementer i $B$. Videre itereres det gjennom alle elementene i $A$ og det blir utført kall på \code{B.inneholder(A.element)} for alle elementer i $A$. Videre har vi to mulige utfall av iterasjonen. 1) Hvis \code{inneholder} returner \code{false} for et av elementene kan søket avsluttes tidlig og det kan konkluderes at $A$ \underline{ikke} er delmengde av $B$, så metoden returnerer \code{false}. 2) Hvis alle elementer har blitt sjekket uten funn av element fra $A$ som ikke er med i $B$ kan det konkluderes at $A$ er delmengde av $B$ og metoden returnerer \code{true}.
...
\oppgaveDel{f} Se vedlagte filer src/Person.java og src/Program.java
